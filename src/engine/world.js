...
/**
 * Villagers who move about, as opposed to villagers doing a job on the spot.
 *
 * Same tethered wander as the livestock, with four rules an animal does not
 * need and a person does:
 *
 *   ONLY THE AMBIENT CAST WALKS. Every goal in the game points at a named
 *   character — Tituba, Ingersoll, Rebecca Nurse, the man at the fence — and
 *   an arrow pointing at a moving target is a bug report waiting to happen.
 *   The woman at the well and the boy with the swine are never a goal, so
 *   they are free to move. `wanders` is opt-in per placement for that reason.
 *
 *   NOBODY WALKS AWAY MID-SENTENCE. An actor in conversation is frozen.
 *
 *   NOBODY STANDS IN A DOORWAY OR ON SOMETHING YOU CAN EXAMINE. A villager
 *   parked on a warp tile or on the woodpile would make the thing the player
 *   was sent to do impossible, and it would look like nothing at all — they
 *   would simply press Z and get a person.
 *
 *   AND NOBODY WALKS THROUGH ANYBODY. Including the player.
 */
export function updateWalkers(map, dt, now, busyId, player) {
  for (const a of map.actors || []) {
    if (!a.def || !a.def.wanders || a.id === busyId) continue;

    if (a.moving) {
      a.t += dt / WALK_STEP;
      if (a.t >= 1) {
        a.t = 0; a.moving = false;
        a.px = a.tx * TS; a.py = a.ty * TS;
        a.animFrame = 0;
        a.wait = 2.5 + hash(a.tx, a.ty, Math.floor(now)) * 7;
      } else {
        a.px = (a.fromX + (a.tx - a.fromX) * a.t) * TS;
        a.py = (a.fromY + (a.ty - a.fromY) * a.t) * TS;
        a.animFrame = [0, 1, 0, 2][(a.steps + (a.t > 0.5 ? 1 : 0)) % 4];
      }
      continue;
    }

    a.wait = (a.wait === undefined ? 1 + hash(a.tx, a.ty, 11) * 4 : a.wait) - dt;
    if (a.wait > 0) continue;

    const r = hash(a.tx * 29 + 5, a.ty * 19 + 11, Math.floor(now * 2));
    const d = Math.floor(r * 4);
    const [dx, dy] = [[0, -1], [1, 0], [0, 1], [-1, 0]][d];
    const nx = a.tx + dx, ny = a.ty + dy;
    const key = `${nx},${ny}`;

    const strayed = Math.abs(nx - a.homeX) + Math.abs(ny - a.homeY) > WALK_RANGE;
    const occupied = (player && player.tx === nx && player.ty === ny)
      || map.actors.some((o) => o !== a && o.tx === nx && o.ty === ny);
    if (strayed || occupied
        || nx < 0 || ny < 0 || nx >= map.w || ny >= map.h
        || isSolid(map, nx, ny)
        || map.warps.has(key) || map.interact.has(key)) {
      a.wait = 1.5 + r * 3;
      // Look where they thought about going. A villager who changes their
      // mind and turns is doing more for the village than one who does not
      // move at all, and it costs nothing.
      a.dir = dx < 0 ? 'left' : dx > 0 ? 'right' : dy < 0 ? 'up' : 'down';
      a.dirIndex = DIR_INDEX[a.dir];
      continue;
    }
...
const WALK_STEP = 0.55;         // seconds per tile — a person, not an animal
const WALK_RANGE = 5;           // tiles from where they started, Manhattan
...
export function renderMap(g, map, cam, actors, clock = 0) {
  ...
  if (!map.indoor) drawWeather(g, map.chapter, clock, VW, VH);
...
/**
 * Weather, drawn as a stateless function of the clock.
 *
 * No particle array and nothing to update: every mote's position is computed
 * from its index and the time, wrapped. That means it costs nothing when the
 * player is indoors, survives a map change without leaking, and cannot drift
 * out of sync with anything.
 *
 * March gets sleet. The first week of March 1692 in Essex County was cold and
 * wet — the roads in this village are drawn as mud for a reason — and the
 * arrival text says the cold "is in the ground, and it comes up". Fine, fast,
 * driven almost sideways.
 *
 * June gets nothing. It is dry, dusty and hot, and the absence is doing work:
 * three chapters that all had weather would read as one place with an effect
 * on it, rather than as three different days.
 *
 * September gets leaves, drifting slowly across a low gold afternoon. Almost
 * still. The village has emptied by then and the weather should feel like
 * nothing is in a hurry any more.
 */
function drawWeather(g, chapter, clock, VW, VH) {
  if (chapter === 'march') {
    // Sleet: short streaks, steep and quick, with two speeds so it has depth.
    g.save();
    g.strokeStyle = 'rgba(198, 214, 236, 0.42)';
    g.lineWidth = 1;
    g.beginPath();
    for (let i = 0; i < 90; i++) {
      const far = i % 3 === 0;
      const speed = far ? 150 : 260;
      const drift = far ? 34 : 58;
      const x = (hash(i, 1, 91) * VW * 1.4 - clock * drift) % (VW + 40);
      const y = (hash(i, 2, 97) * VH + clock * speed) % (VH + 20);
      const len = far ? 3 : 5;
      g.moveTo(x, y);
      g.lineTo(x + len * 0.34, y + len);
    }
    g.stroke();
    g.restore();
    return;
  ...
  if (chapter === 'september') {
    // Leaves. Few, slow, and each one drifting on its own sine so they do
    // not fall as a sheet.
    // Contrast matters more than accuracy here: a brown leaf over brown mud
    // under an amber season wash is invisible. Two of the four are lifted
    // well above the ground tone so some of them always read.
    const COLOURS = ['#d8a758', '#8a5626', '#c68f3e', '#6d4520'];
    for (let i = 0; i < 26; i++) {
      const speed = 16 + hash(i, 5, 71) * 14;
      const y = (hash(i, 6, 73) * VH + clock * speed) % (VH + 12);
      const sway = Math.sin(clock * (0.5 + hash(i, 7, 79) * 0.6) + i) * 11;
      const x = (hash(i, 8, 83) * VW + clock * 9 + sway) % (VW + 12);
      g.fillStyle = COLOURS[i % COLOURS.length];
      g.globalAlpha = 0.92;
      // Three pixels, tilted, so it reads as a leaf turning over rather than
      // as a speck of dirt.
      g.fillRect(Math.floor(x), Math.floor(y), 2, 1);
      g.fillRect(Math.floor(x) + (i % 2 ? 1 : -1), Math.floor(y) + 1, 2, 1);
      g.globalAlpha = 1;
    }
  }
}
