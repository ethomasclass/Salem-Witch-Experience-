...
export const PUTNAMHOUSE = {
  ...
  interact: [
    // One gate, and it used to be two.
    //
    // The second was the boundary stone: the idea was that a page of
    // surveyors' language means nothing until you have stood on the line it
    // argues about. Good idea, wrong order — once March became a chain, the
    // ledger sends the player to the Putnams BEFORE the woods, so the goal
    // read "read it, on their own table" and the table was empty. A student
    // reported it from the classroom, which is the worst way to find a bug.
    //
    // The chain's order turns out to be the better one anyway: read the claim
    // first, then walk north and stand on the ground it is about.
    { id: 'putnamTable', doc: 'topsfieldPetition', x: 2, y: 4, w: 2, h: 1,
      require: ['paper.topsfield'] },
  ],
